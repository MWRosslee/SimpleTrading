...
def main():
    ...
    # Make 90 day future predictions
    forecast = 90
    future_dates = pd.date_range(start=data.index[-1], periods=forecast, freq='D')
    future_data = pd.DataFrame(index=future_dates, columns=X.columns)
    future_data.fillna(method='ffill', inplace=True)
    preds = clf.predict(future_data)

    # Replace NaN values with forward-filled values
    ...
    support = min(data['Close'][-90:])
    resistance = max(data['Close'][-90:])
    plt.hlines(support, xmin=data.index[-90], xmax=future_dates[-1], colors='k', linestyles='--')
    plt.hlines(resistance, xmin=data.index[-90], xmax=future_dates[-1], colors='k', linestyles='--')

    # Plot predictions
    plt.scatter(future_dates, preds, marker='^', c='g', label='Predictions')

    plt.title(f"{ticker_symbol} Next {forecast} Days Predictions")
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.legend()
    plt.show()
